...
module.exports.run = async (_tasks, config) => {
  ...
  Object.keys(_tasks).forEach(text => {
    ...
    if (
      typeof _tasks[text].promise === "undefined" &&
      typeof _tasks[text].command === "undefined"
    )
      throw new Error(
        `Task '${text}' has no promise or command specified! At least one must be specified`
      );
  ...
  const p = {
    ...
    addSubmessage: function({ task, message }) {
      for (let i = 0; i < this.lines.length; i++) {
        ...
        if (line.name === task.name) {
          if (!this.lines[i].sub) this.lines[i].sub = "";
    ...
    render: function() {
      ...
      for (let i = 0; i < this.lines.length; i++) {
        ...
        switch (line.state) {
          case this.states.PROGRESS:
            ui.writeLine(
              chalk.bold(chalk.yellow(figures.pointer) + " " + line.name)
            );
      ...
      ui.writeLine(chalk.grey("_____________________"));
      if (inProgressLine) ui.startProgress(inProgressLine.name + " ");
    ...
    try {
      ...
      if (task.prompt) {
        ...
        if (typeof task.prompt === "string") {
          ...
          promptResponse = await inquirer
            .prompt(task.prompt)
            .then(r => r[responseName]);
        } else {
        ...
        if (promptResponse) {
          p.addSubmessage({
            task,
            message: chalk.reset(`${figures.tick} Passed values from prompt`)
          });
        } else {
          p.addSubmessage({
            task,
            message: chalk.reset(
              `${chalk.red(figures.cross)} Passed values from prompt`
            )
          });
        }
      }
      ...
      if (response && typeof response === "string") {
        p.addSubmessage({
          task,
          message: chalk.reset(`${figures.tick} ` + response)
        });
      }
      ...
      if (task.command) {
        ...
        if (typeof task.command === "string") {
          ...
          await exec(task.command, task.options || {}).catch(
            ({ err, code }) => {
              ...
              message =
                ...
                ((message.split("\n").length > NUM_OUTPUT_LINES &&
                  "\n\t" +
                    chalk.white.italic(
                      `(Showing only last ${NUM_OUTPUT_LINES} lines)`
                    )) ||
                  "");

              throw `${(task.error &&
                task.error.length &&
                chalk.bold(" " + task.error + " ")) ||
                `Exit Code ${code}`}\n\t${chalk.white(
                chalk.bold.underline(task.command) +
                  " output:\n" +
                  chalk.grey(message)
              )}`;
            }
      ...
      if (task.exists) {
        ...
        if (typeof task.exists === "string") {
          ui.startProgress(
            task.name +
              chalk.grey(` [Checking file '${task.exists}' exists...] `)
          );
          ...
          if (!exists) throw new Error(`File '${task.exists}' does not exist!`);
            ...
            p.addSubmessage({
              task,
              message: chalk.reset(
                `${figures.tick} File '${task.exists}' exists`
              )
            });
        }
      } //end if task.exists
      ...
      if (errMessage) p.addSubmessage({ task, message: errMessage });
